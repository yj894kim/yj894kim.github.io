(() => {
  ...
  function update(delta) {
    ...
    for (const enemy of state.enemies) { const distance = Math.hypot(p.x - enemy.x, p.y - enemy.y); if (distance < p.size + enemy.size) { const sizeDifference = p.size - enemy.size; if (sizeDifference > 0.01) { updateScore(state.score + Math.round(enemy.size * 3)); spawnParticles(enemy.x, enemy.y, enemy.color); enemy.x = random(25, canvas.width - 25); enemy.y = random(25, canvas.height - 25); enemy.size = random(9, 42); } else if (sizeDifference < -0.01) { spawnParticles(p.x, p.y, colors[state.character]); endGame('A larger worm caught you. Smaller worms are safe to eat.'); break; } } }
